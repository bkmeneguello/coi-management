...
ALTER TABLE ENTRADA DROP COLUMN VALOR;
ALTER TABLE ENTRADA DROP COLUMN DESCONTO;
ALTER TABLE ENTRADA_PRODUTO ADD COLUMN VALOR DECIMAL(8,2);
UPDATE ENTRADA_PRODUTO SET VALOR = (SELECT PRECO FROM PRODUTO WHERE ID = ENTRADA_PRODUTO.PRODUTO_ID);
ALTER TABLE ENTRADA_PRODUTO ALTER COLUMN VALOR SET NOT NULL;
ALTER TABLE ENTRADA_PRODUTO ADD COLUMN DESCONTO DECIMAL(8,2) DEFAULT 0;

ALTER TABLE COMISSAO DROP CONSTRAINT COMISSAO_FK_PARTE;
ALTER TABLE COMISSAO ADD COLUMN PARTE VARCHAR(17);
UPDATE COMISSAO SET PARTE_ID = 6 WHERE PARTE_ID = 2 AND DESCRICAO = 'Realizador';
UPDATE COMISSAO SET PARTE = DECODE(PARTE_ID, 1, 'CONSULTORIO', 2, 'MEDICO', 3, 'FISIOTERAPEUTA', 4, 'PACIENTE', 5, 'CLIENTE', 6, 'MEDICO_REALIZADOR');
UPDATE COMISSAO SET PARTE = TRIM(PARTE);
ALTER TABLE COMISSAO ALTER COLUMN PARTE SET NOT NULL;
ALTER TABLE COMISSAO DROP COLUMN PARTE_ID;
ALTER TABLE COMISSAO DROP COLUMN DESCRICAO;
ALTER TABLE COMISSAO DROP COLUMN VALOR;
ALTER TABLE COMISSAO ADD CONSTRAINT COMISSAO_PARTE_UK UNIQUE (CATEGORIA_ID, PARTE);

ALTER TABLE ENTRADA_PARTE DROP CONSTRAINT ENTRADA_PARTE_FK_PARTE;
ALTER TABLE ENTRADA_PARTE ADD COLUMN PARTE VARCHAR(17);
UPDATE ENTRADA_PARTE SET PARTE_ID = 6 WHERE PARTE_ID = 2 AND DESCRICAO = 'Realizador';
UPDATE ENTRADA_PARTE SET PARTE = DECODE(PARTE_ID, 1, 'CONSULTORIO', 2, 'MEDICO', 3, 'FISIOTERAPEUTA', 4, 'PACIENTE', 5, 'CLIENTE', 6, 'MEDICO_REALIZADOR');
UPDATE ENTRADA_PARTE SET PARTE = TRIM(PARTE);
ALTER TABLE ENTRADA_PARTE ALTER COLUMN PARTE SET NOT NULL;
ALTER TABLE ENTRADA_PARTE DROP COLUMN PARTE_ID;
ALTER TABLE ENTRADA_PARTE DROP COLUMN DESCRICAO;
ALTER TABLE ENTRADA_PARTE ADD CONSTRAINT ENTRADA_PARTE_PARTE_UK UNIQUE (ENTRADA_ID, PARTE, PESSOA_ID);

DROP TABLE PESSOA_PARTE;
DROP TABLE PARTE;